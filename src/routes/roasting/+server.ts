    ...
    for (let x = 0; x < includes.length; x++) {
        ...
        if (inc.components != undefined) {
            for (let i = 0; i < inc.components.elements.length; i++) {
                ...
                if (element.entityComponent.titleV2 != undefined) {
...
export const POST: RequestHandler = async (event) => {
    ...
    try {
        ...
        let prompt = "sebagai candaan, tolong roasting profil linkedin berikut dengan roastingan yang kejam dan menyindir dan pakai bahasa gaul dan gunakan emotikon bila perlu, " + infoIndo(biodata) + ". (hanya berikan response dengan singkat dalam bahasa indonesia)";
        if (language == "English") {
            prompt = "as a joke, please roast linkedin profile below with harshly and sarcastic with slang language and use emoticon if needed, " + infoInggris(biodata) + ". (only give short response with english language)";
        }
